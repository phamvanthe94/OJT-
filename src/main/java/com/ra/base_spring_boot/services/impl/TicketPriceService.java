...
public class TicketPriceService {
    @Autowired
    private ITicketPriceRepository ITicketPriceRepository;
    ...
    public ResponseEntity<ResponseWrapper<Page<TicketPrice>>> getAllAndSearchTicketPrice(Boolean dayType,
        ...
        Page<TicketPrice> ticketPricePage = ITicketPriceRepository.search(dayType, seatTypeEnum, movieTypeEnum, pageable);
    ...
    public ResponseEntity<ResponseWrapper<?>> createTicketPrice(TicketPriceDTO ticketPriceDTO, BindingResult bindingResult) {
        if (ticketPriceDTO == null) {
            ...
            TicketPrice newTicketPrice = ITicketPriceRepository.save(ticketPrice);
    ...
    public ResponseEntity<ResponseWrapper<?>> updateTicketPrice(Long id, TicketPriceDTO ticketPriceDTO) {
        TicketPrice oldTicketPrice = ITicketPriceRepository.findById(id).orElseThrow(() -> new RuntimeException("Ticket Price not found with id: " + id));
        if (ticketPriceDTO != null) {
            TicketPrice ticketPrice = convertTicketPriceDTOToTicketPrice(ticketPriceDTO);
            ticketPrice.setId(oldTicketPrice.getId());
            TicketPrice updatedTicketPrice = ITicketPriceRepository.save(ticketPrice);
            ResponseWrapper<TicketPrice> responseWrapper = ResponseWrapper
                    .<TicketPrice>builder()
    ...
    public ResponseEntity<ResponseWrapper<String>> deleteTicketPrice(Long id) {
        TicketPrice ticketPrice = ITicketPriceRepository.findById(id).orElseThrow(() -> new RuntimeException("Ticket Price not found with id: " + id));
        ITicketPriceRepository.delete(ticketPrice);
